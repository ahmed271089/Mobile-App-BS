...
import { getPost, markPostSolved, toggleLikePost, toggleFavoritePost } from '../../api/posts';
...
export default function PostDetailScreen({ navigation, route }: any) {
  ...
  const handleMarkSolved = async () => {
    const humanComments = (post?.comments ?? []).filter((c: ApiComment) => !c.isAIComment);
    if (humanComments.length === 0) {
      Alert.alert('No comments', 'Wait for a community reply before marking as solved.');
      return;
    }
  ...
  const handleToggleLike = async () => {
    try {
      ...
      setPost((prev: any) => (prev ? { ...prev, likesCount: prev.likesCount + (result.liked ? 1 : -1) } : prev));
    } catch {
      ...
      <View style={[styles.header, { paddingTop: insets.top + spacing.lg }]}>
        ...
        <View style={styles.headerActions}>
          ...
          <Pressable style={styles.backBtn} onPress={handleToggleFavorite}>
            <Ionicons name={favorited ? 'bookmark' : 'bookmark-outline'} size={18} color={favorited ? colors.primary : colors.textSecondary} />
          </Pressable>
      ...
      <View style={styles.content}>
        ...
        {(post.comments ?? []).map((c: ApiComment) => (
          <View key={c.id} style={styles.commentRow}>
            <View style={styles.commentAvatar}>
              <Text style={styles.commentAvatarText}>{c.author.name.slice(0, 2).toUpperCase()}</Text>
            </View>
            <View style={{ flex: 1 }}>
        ...
        <View style={styles.commentInputRow}>
          <TextInput
            ...
            multiline
          />
          <Pressable onPress={handleAddComment} style={styles.sendCommentBtn} disabled={submitting}>
            <Ionicons name="send" size={16} color={colors.white} />
          </Pressable>
        </View>

        {post.status !== 'SOLVED' && post.type === 'PROBLEM' && (
          <Button label="Mark as Solved" variant="secondary" style={{ marginTop: spacing.xl }} onPress={handleMarkSolved} />
        )}
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: spacing.lg, marginBottom: spacing.lg },
  headerActions: { flexDirection: 'row', gap: spacing.sm },
  backBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.card, alignItems: 'center', justifyContent: 'center' },
  content: { paddingHorizontal: spacing.lg },
  badgeRow: { flexDirection: 'row', gap: spacing.xs, marginBottom: spacing.md },
  title: { ...typography.h1, color: colors.textPrimary, marginBottom: spacing.sm },
  description: { ...typography.body, color: colors.textSecondary, lineHeight: 20, marginBottom: spacing.lg },
  aiBox: { backgroundColor: colors.infoMuted, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.primaryMuted, padding: spacing.lg, marginBottom: spacing.xl, gap: spacing.sm },
  aiText: { ...typography.caption, color: colors.textSecondary, lineHeight: 17 },
  ...
  commentAvatarText: { ...typography.caption, color: colors.primary, fontWeight: '700' },
  commentMeta: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, flexWrap: 'wrap' },
  commentAuthor: { ...typography.bodyBold, color: colors.textPrimary },
  commentTime: { ...typography.caption, color: colors.textMuted },
  commentText: { ...typography.caption, color: colors.textSecondary, marginTop: 2 },
  commentInputRow: { flexDirection: 'row', alignItems: 'flex-end', gap: spacing.sm, marginTop: spacing.md },
  commentInput: { flex: 1, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.cardBorder, borderRadius: radius.lg, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, color: colors.textPrimary, maxHeight: 80 },
  sendCommentBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center' },
});
